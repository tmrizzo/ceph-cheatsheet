...
### Admin keyring /etc/ceph/ceph.client.admin.keyring permission
...
- This configuration is important for S3 buckets and Swift containers creation. The API follows the pattern `bucketname.hostname.fqdn` i.e `mybucket.node4.lab.example.com`. The arbitrary subdomain name resolution must be available, otherwise, the bucket or container *specific name* must be added to the DNS.
...
- Packages:
  ...
  Description: Choose an OpenStack version available in the repository.
...
- Description: Instead of 'all' can also specify `<param>` name.
...
### Dump stuck placement groups, if any.  Threshold is the cutoff after which a pg is returned as stuck, with a default of 300s.
  ...
  - Description: The output will contain the placement group names, their full state and which OSDs hold (or held) the data. Also see [here](http://docs.ceph.com/docs/luminous/rados/operations/pg-states/) for details on pg states.
...
- Subcommands of **`ceph daemon <daemonname>`**
...
### Dump a json list of currently active operations for an OSD. Useful if one or more ops are stuck
...
### Delete a user (likely will also need to remove key material)
...
- Description: RADOS Object Store utility invocations
...
### Download an object from a pool into a local file. Give '-' as a file name to write to **stdout**
...
### List watchers of an object in pool. For instance, the head object of a mapped rbd volume has it's clients as watchers
...
- Description: **`--caps=<caps>`** - list of caps e.g., `usage=read, write; user=read`.
  ...
  - **`caps="users=read,write"`** - read/wirte permission to change users
  - **`caps="buckets=read,write"`** -  read/wirte permission to change buckets
  - **`caps="zones=read,write"`** - read/wirte permission to change zones
...
- Command: **`radosgw-admin user info --uid=username`**
...
### Trim usage information between two dates. The ISO date format (YYYY-MM-DD) can be used to specify dates
...
- Command:  **`radosgw-admin bucket rm --bucket=mysecondbucket`**
...
- Command: **`s3cmd mb s3://bucketname`**
  ...
  - **Warning: The lab must be using a name resolution, this one uses Dnsmasq. If not, the bucket creation will fail with the name resolution**
